...
//
//    Description:  copied from jasson's json2pb
//
//        Version:  1.0
//        Created:  05/21/2014 04:18:16 PM
//       Revision:  none
//       Compiler:  g++
//
...
static bool _json2field(Message &msg, const FieldDescriptor *field, cJSON *json)
{
    ...
    switch(field->cpp_type())
    {
        case FieldDescriptor::CPPTYPE_INT32:
            ...
            {
                WARNING("[%s] pb is int32, json is not a number", field->name().c_str());
                return false;
            }
            if (repeated)
        ...
        case FieldDescriptor::CPPTYPE_INT64:
            if (cJSON_Number != json->type)
            {
                WARNING("[%s] pb is int64, json is not a number", field->name().c_str());
                return false;
            }
            if (repeated)
        ...
        case FieldDescriptor::CPPTYPE_UINT32:
            if (cJSON_Number != json->type)
            {
                WARNING("[%s] pb is uint32, json is not a number", field->name().c_str());
                return false;
            }
            if (repeated)
        ...
        case FieldDescriptor::CPPTYPE_UINT64:
            if (cJSON_Number != json->type)
            {
                WARNING("[%s] pb is uint64, json is not a number", field->name().c_str());
                return false;
            }
            if (repeated)
        ...
        case FieldDescriptor::CPPTYPE_DOUBLE:
            if (cJSON_Number != json->type)
            {
                WARNING("[%s] pb is double, json is not a number", field->name().c_str());
                return false;
            }
            if (repeated)
        ...
        case FieldDescriptor::CPPTYPE_FLOAT:
            if (cJSON_Number != json->type)
            {
                WARNING("[%s] pb is float, json is not a number", field->name().c_str());
                return false;
            }
            if (repeated)
        ...
        case FieldDescriptor::CPPTYPE_BOOL:
            ...
            {
                WARNING("[%s] pb is bool, json is not", field->name().c_str());
                return false;
            }
            if (repeated)
        ...
        case FieldDescriptor::CPPTYPE_ENUM:
            {
                ...
                if (NULL == ed)
                {
                    WARNING("failed to get enum descriptor");
                    return false;
                }
                if (cJSON_Number == json->type)
                {
                    ...
                    if (NULL == ev)
                    {
                        WARNING("failed to find enum by value: %d", json->valueint);
                        return false;
                    }
                }
                else if (cJSON_String == json->type)
                {
                    ...
                    if (NULL == ev)
                    {
                        WARNING("failed to find enum by name: %s", json->valuestring);
                        return false;
                    }
                }
                else
                {
                    WARNING("[%s] pb is enum, json is not", field->name().c_str());
                    return false;
                }
                if (repeated)
        ...
        case FieldDescriptor::CPPTYPE_STRING:
            ...
            {
                WARNING("[%s] pb is string, json is not", field->name().c_str());
                return false;
            }
            if (repeated)
...
static bool _json2pb(Message &msg, cJSON *json)
{
    ...
    {
        WARNING("des[%p] or ref[%p] is NULL", des, ref);
        return false;
    }
    for (cJSON *chld = json->child; NULL != chld; chld = chld->next)
    {
        if (NULL == chld->string)
        {
            WARNING("sub item does not have name");
            return false;
        }
        ...
        if (NULL == field)
        {
            WARNING("failed to get field by name[%s]", chld->string);
            return false;
        }
        if (field->is_repeated())
        {
            ...
            {
                WARNING("pb field[%s] is array, but json is not", chld->string);
                return false;
            }
