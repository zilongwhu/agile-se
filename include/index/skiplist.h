...
template<typename TMemoryPool = Mempool, uint8_t M = 20>
class TSkipList
{
    private:
    ...
    public:
        static int init_pool(DelayPool *pool, uint16_t payload_len)
        {
            for (uint32_t i = 0; i < M; ++i)
            {
                ...
                {
                    P_WARNING("failed to register node, level=%u, size=%u, payload_len=%hu", i, size, payload_len);
                    return -1;
                }
                P_WARNING("register node ok, level=%u, size=%u, payload_len=%hu", i, size, payload_len);
            }
            P_WARNING("init pool ok, payload_len=%hu", payload_len);
            return 0;
        }
    private:
    ...
    public:
        ...
        bool insert(int id, void *payload)
        {
            ...
            if (vcur)
            {
                if (m_payload_len > 0 && ::memcmp(payload, old_payload, m_payload_len))
                {
                    ...
                    if (0 == vnew)
                    {
                        P_WARNING("failed to alloc renew node, id=%d, level=%u, node_size=%u", id, level, node_size);
                        return false;
                    }
                    ...
                    P_DEBUG("overwrite node ok, id=%d, level=%u, node_size=%u", id, level, node_size);
                }
            }
            else
            {
                ...
                if (0 == vnew)
                {
                    P_WARNING("failed to alloc new node, id=%d, level=%u, node_size=%u", id, level, node_size);
                    return false;
                }
                node_t *pnew = (node_t *)m_pool->addr(vnew);
                pnew->id = id;
                ...
                P_DEBUG("insert new node ok, id=%d, level=%u, node_size=%u", id, level, node_size);
            }
            return true;
        }
        void remove(int id)
        {
            ...
            if (vcur)
            {
                ...
                --m_size;
                m_pool->delay_free(vcur, node_size);
                P_DEBUG("delete node ok, id=%d, level=%u, node_size=%u", id, level, node_size);
            }
        }
    public:
